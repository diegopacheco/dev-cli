...
var helpSections = []helpSection{
	...
	{"MODES", theme.Magenta, []helpRow{
		{"-sql --postgres QUERY", "run SQL on Postgres · all lists every object and command"},
		{"-sql --mysql QUERY", "run SQL on MySQL · all"},
		{"--sqlite QUERY", "run SQL on SQLite (also --sqllite) · all"},
		{"-cassandra QUERY", "run CQL (also -cql) · all"},
		{"-redis COMMAND", "run Redis commands, one per line"},
		{"-loki LOGQL", "query Loki · all · labels · values LABEL · :range 30m · :limit 100"},
		{"-grafana COMMAND", "all · chart UID [PANEL] · health · search · dashboard UID · datasources · query DS EXPR · get /api/..."},
	...
	{"QUICK RUNS", theme.Purple, []helpRow{
		...
		{`devcli -loki all`, ""},
		{`devcli -sql --postgres all`, ""},
		{`devcli -grafana "chart devcli-metrics"`, ""},
